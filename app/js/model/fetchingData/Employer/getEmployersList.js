...
import EmployerList from '../../Components/Employer/EmployerListComponent'
    ...
    if (employersWrapper) {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (!employers) {
                    throw new Error('Щось пішло не так, роботодавець не знайдений, обновіть сторінку, будь-ласка')
                }
                ...
                if (scroll) {
                    ...
                    if (!avoidFetch) {
                        empList.update([])
                        throw new EmptyError('Список роботодавців пустий')
                    } else {
