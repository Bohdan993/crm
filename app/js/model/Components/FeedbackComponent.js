    ...
    constructor() {
        this.el = el('div.input-group',
            ...
            this.label = el('label', {
                ...
                this.text = el('span', 'Інформаційний'))
        )
        ...
        this.form.addEventListener('submit', (e) => {
            ...
            if (this.inputClient.checked) {
                if (this.findClient.value.trim() === '') {
                    toastr.error(`Виберіть клієнта`, 'Помилка!')
                    this.parent.changechoiseClientText('Вибрати')
                    return
                }
    ...
    constructor() {
        ...
        this.el = el('form.modal-row__feedback-row.add-feedback-form',
            el('div.modal-row__feedback-speakers',
                ...
                this.choiseClient = el('a.modal-row__feedback-choise', 'Вибрати'),
            ...
            el('div.modal-row__feedback-controls',
                el('div.modal-row__feedback-controls_left',
                    this.cancel = el('button.modal-row__feedback-cancel.feedback-btn', 'Відмінити'),
                    this.saveFeedback = el('button.modal-row__feedback-save.feedback-btn', 'Зберегти')
                ),
                el('div.modal-row__feedback-controls_right',
                    this.delete = el('button.modal-row__feedback-delete.delete-btn', 'Видалити')
                )
            )
        )
        ...
        this.delete.addEventListener('click', (e) => {
            e.preventDefault()
            let conf = confirm(`Підтвердіть видалення відгуку`)
            if (this.data.type === 'employer') {
                if (conf) {
                    deleteFeedback({
                        str: 'employers',
        ...
        this.el = el('div',
            this.row = place(el('div.modal-row__feedback-row',
                ...
                el('div.modal-row__feedback-date',
                    ...
                    this.edit = el('button.modal-row__feedback-edit.edit-btn', 'Редагувати')
                ),
...
export default class Feedback {
    constructor(type) {
        ...
        this.controls = el('div.modal-row__controls',
            el('p', 'Відгуки',
                ...
                this.feedbackCountNeg = el('span.negative', 'негативних - 0'),
                this.closeBraket = place(el('span', ')'))
            ),
            this.addItem = el('div.add-item', el('span', '+'), 'додати відгук')
        )
    ...
    update(data, context) {
        ...
        if (data.data && data.data.length < data.total) {
            this.showMore.update(true, 'показати ще')
        ...
        setAttr(this.feedbackCountNeg, {
            innerText: data.badFeedback !== 0 ? `негативних - ${data.badFeedback || 0}` : ''
        })
