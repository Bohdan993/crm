...
export default class FindEmployerPopupComponent {
    ...
    constructor(type) {
        ...
        this.el = el('div.vacancy-modal-popup#employer-type-popup',
            this.form = el('form',
                el('p.vacancy-modal__title', `Введіть ім'я роботодавця
						або назву підприємства`),
        ...
        this.autocomplete = new Autocomplete(this.searchGroup, {
                ...
                try {
                    ...
                    return this.data.filter(employer => {
                        return employer.name.toLowerCase()
                            ...
                            employer.enterprise && employer.enterprise.toLowerCase()
                                .includes(input.toLowerCase())
                    })
                } catch (err) {
                    console.error(err)
                ...
                return `
				    <li ${props}>
				      <div class="wiki-title">
				        ${((result?.name) || '') + (result.name && result.enterprise ? '\u00A0 - \u00A0' : '') + (result?.enterprise || '')}
				      </div>
				    </li>
				  `
            },
            getResultValue: result => {
                this.result = result
                return `${((result?.name) || '') + (result.name && result.enterprise ? '\u00A0 - \u00A0' : '') + (result?.enterprise || '')}`
            },
            autoSelect: true
