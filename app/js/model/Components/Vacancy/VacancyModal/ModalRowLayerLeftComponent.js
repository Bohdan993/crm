    ...
    constructor() {
        ...
        this.form.addEventListener('submit', (e) => {
            ...
            setAttr(this.parent.products, {
                style: {
                    backgroundColor: text === `Практика` ? '#9c3' : text === 'Сезонна' ? '#e37373' : text === 'Канікули' ? '#decd30' : '#39c'
                }
            })
...
export default class ModalRowLayerLeft {
    ...
    update(data, context) {
        ...
        d.setDate(+d.getDate() + +data.period_day)
        ...
        if (context === 'nulledEmployer') {
            ...
            setAttr(this.dates, {
                innerText: data.period || data.period_day ? `${data.startWork} - ${formatDate(d)}` : '-'
            })
        }
        ...
        setAttr(this.visaType, {
            innerText: data.type_production ? (data.type_vacancy === '1' ? `Сезонна - ${'\u00A0'}` : data.type_vacancy === '2' ? `Практика - ${'\u00A0'}` : data.type_vacancy === '3' ? `Робоча - ${'\u00A0'}` : `Канікули - ${'\u00A0'}`) : 'Оберіть тип продукції'
        })
        ...
        setAttr(this.products, {
            style: {
                backgroundColor: data.type_production ? (data.type_vacancy === '2' ? '#9c3' : data.type_vacancy === '1' ? '#e37373' : data.type_vacancy === '3' ? '#39c' : '#decd30') : '#85a6cb'
            }
        })
        ...
        setAttr(this.period, {
            innerText: `${data.period ? data.period + ' міс' : ''} ${data.period_day ? data.period_day + ' дн' : ''}` || '-'
