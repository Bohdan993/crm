...
export default class RowVacancy {
    ...
    update(data, index, items, context) {
        ...
        setAttr(this.term, {
            innerText: `${data.start_work ? data.start_work : ''} ${data.finish_work ? '- ' +  data.finish_work : ''} (${data.period ? data.period + ' міс' : ''} ${data.period_day ? data.period_day + ' дн' : ''})`
        })
        ...
        setAttr(this.typeVacancy, {
            classList: data.type_vacancy === '1' ? 'label hot__label' : data.type_vacancy === '2' ? 'label green__label' :  data.type_vacancy === '3' ? 'label blue__label' : data.type_vacancy === '4' ? 'label yellow__label' : null
        })

        setAttr(this.typeVacancyName, {
            innerText: data.type_vacancy === '1' ? 'Сезонна' : data.type_vacancy === '2' ? 'Практика' : data.type_vacancy === '3' ? 'Робоча' : data.type_vacancy === '4' ? 'Канікули' : ''
        })
