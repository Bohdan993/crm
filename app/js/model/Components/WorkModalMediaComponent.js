...
import ProgressBar from './Employer/ProgressBarComponent'
import ShowMoreBtn from './Employer/WorkModal/ShowMoreBtnComponent'
import ShowLessBtn from './Employer/WorkModal/ShowLessBtnComponent'
...
export default class WorkModalMedia {
            ...
            try {
                for (let i = 0; i < files.length; i++) {
                    ...
                    if (/\jpe?g|png$/i.test(str[1]) === false) {
                        throw new Error("Завантажений файл має бути картинкою")
    ...
    update(data, index) {
        ...
        if (data.data.length < data.total) {
            ...
            if (this.open) {
                this.showLess.update(true, 'приховати')
            } else {
                this.showLess.update(false)
            }
